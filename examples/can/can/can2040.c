...
static void pio_sync_setup(struct can2040 *cd) {
    ...
    sm->execctrl = (gpio_rx << PIO_SM0_EXECCTRL_JMP_PIN_LSB | (can2040_offset_sync_end - 1) << PIO_SM0_EXECCTRL_WRAP_TOP_LSB | can2040_offset_sync_signal_start << PIO_SM0_EXECCTRL_WRAP_BOTTOM_LSB);
    sm->pinctrl = (1 << PIO_SM0_PINCTRL_SET_COUNT_LSB | gpio_rx << PIO_SM0_PINCTRL_SET_BASE_LSB);
...
static void pio_tx_setup(struct can2040 *cd) {
    ...
    sm->execctrl = (gpio_rx << PIO_SM0_EXECCTRL_JMP_PIN_LSB | can2040_offset_tx_conflict << PIO_SM0_EXECCTRL_WRAP_TOP_LSB | can2040_offset_tx_conflict << PIO_SM0_EXECCTRL_WRAP_BOTTOM_LSB);
    sm->shiftctrl = (PIO_SM0_SHIFTCTRL_FJOIN_TX_BITS | PIO_SM0_SHIFTCTRL_AUTOPULL_BITS);
    uint32_t pinctrl = 0;
    if (cd->gpio_tx >= 0)
        pinctrl = (1 << PIO_SM0_PINCTRL_SET_COUNT_LSB | 1 << PIO_SM0_PINCTRL_OUT_COUNT_LSB | gpio_tx << PIO_SM0_PINCTRL_SET_BASE_LSB | gpio_tx << PIO_SM0_PINCTRL_OUT_BASE_LSB);
    sm->pinctrl = pinctrl;
...
void can2040_start(struct can2040 *cd, uint32_t sys_clock, uint32_t bitrate, int32_t gpio_rx, int32_t gpio_tx) {
    if (IS_RP2350 && ((gpio_rx < 16 && gpio_tx > 31) || (gpio_rx > 31 && gpio_tx < 16)))
        gpio_tx = -1;
    cd->gpio_rx = gpio_rx;
    cd->gpio_tx = gpio_tx;
    data_state_clear_bits(cd);
